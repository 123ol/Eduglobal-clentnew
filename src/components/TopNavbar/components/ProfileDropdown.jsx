...
const ProfileDropdown = ({ className }) => {
  ...
  const { user, removeSession } = useAuthContext(); // ✅ Destructure user here

  // If no user, don't show dropdown
  if (!user) return null;
  const themeModes = [{
  ...
  const editProfileLink = user.role === 'admin' ? '/admin/edit-profile' : '/student/edit-profile';
  const accountSettingsLink = user.role === 'admin' ? '/admin/admin-settings' : '/student/setting';
      ...
      <DropdownMenu as="ul" className="dropdown-animation dropdown-menu-end shadow pt-3" aria-labelledby="profileDropdown">
        ...
        <li className="px-3 mb-3">
          <div className="d-flex align-items-center">
            ...
            <div>
              <Link className="h6" to={dashboardLink}>
                {user.name}
              </Link>
        ...
        {/* Links */}
        <li>
          <DropdownItem as={Link} to={editProfileLink}>
        ...
        <li>
          <DropdownItem as={Link} to={accountSettingsLink}>
